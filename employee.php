...
<form method="POST" action="deliverable2.php"> 
...
<p> Update the name by inserting the old and new values below, will only update persons who are above 18 years of age: </p>
...
<!-- Create a form to pass the value.  
     See below for how to get the value. --> 

<p> Delete a user by their ID number if they are above 18 years of age: </p>
<p><font size="2"> Number </font></p>
<form method="POST" action="deliverable2.php">
<!-- refreshes page when submitted -->

   <p><input type="text" name="deleteID" size="6">
<!-- Define one variable to pass value. -->
...
if ($db_conn) {
	...
	if (array_key_exists('reset', $_POST)) {
		...
		if (array_key_exists('insertsubmit', $_POST)) {
			...
			if (array_key_exists('updatesubmit', $_POST)) {
				...
				executeBoundSQL("update tab1 set name=:bind2 where name=:bind1 AND age>18", $alltuples);
				OCICommit($db_conn);
				...
				executeBoundSQL("delete from tab1 where nid=:bind1 AND age>18", $alltuples);
				OCICommit($db_conn);
			} else
	...
	if ($_POST && $success) {
		...
		header("location: deliverable2.php");
	} else {
		// Select data...
		$result = executePlainSQL("select * from tab1");
		/*printResult($result);*/
           /* next two lines from Raghav replace previous line */
           $columnNames = array("Customer ID#", "First Name", "Age");
